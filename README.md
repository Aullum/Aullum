...
7+ years of experience designing, implementing and operating production backends.

**Core focus:** Python · FastAPI · async integrations · PostgreSQL · Redis · ETL/data pipelines · Docker · CI/CD · observability

---

## At a glance

- 7+ years building and running backends for a US e-commerce product company (remote since 2019)
- Built and evolved backend platforms for warehouse operations, inventory, pricing, analytics, and marketplace synchronization
- Own and evolve a unified FastAPI + PostgreSQL platform for warehouse operations, inventory, pricing and marketplace integrations
- Built systems that keep **tens of thousands of physical items** and **hundreds of thousands of SKUs** in sync across Amazon, eBay, Walmart, Shopify, and other channels
- Designed and implemented a separate async ETL service using **Hexagonal architecture**
- Introduced monitoring and observability with **Prometheus + postgres-exporter + Grafana + health checks**
- Recent client work also includes **ML tooling**, **Circle → Hivebrite migration**, and **marketplace product development**
...
- Hexagonal architecture for a separate ETL service
...
- Migrated critical workflows from Google Sheets / Apps Script / JS / PHP / Excel formulas into a maintainable backend platform
- Designed and implemented core services for inventory, product catalog, pricing and analytics
- Built marketplace synchronization flows across Amazon, eBay, Walmart, Newegg, Shopify, and ShipStation
- Ensured data consistency for **tens of thousands of physical items** and **hundreds of thousands of SKUs** across multiple marketplace accounts
...
- Introduced monitoring and observability stack (Prometheus + postgres-exporter + Grafana + health checks), improving issue detection and production support
- Worked closely with operations and management to translate real warehouse and sales workflows into robust backend features
...
- Designed with Hexagonal architecture to keep domain logic isolated from API / DB / integration details
...
### Client Projects (Jellyfish.tech)

Worked across multiple client projects in different domains, including ML tooling, platform migration, and marketplace product development.

Highlights:

- Improved a FastAPI-based backend platform for ML model analysis, training workflows, and statistics collection
- Integrated Marimo, improved internal admin tooling with Starlette Admin, and helped stabilize the backend codebase
- Introduced CI, automated tests, linting, formatting, type checks, and pre-commit workflows
- Supported API-driven migration from Circle to Hivebrite using APIs, Google Sheets, and Apps Script
- Delivered backend fixes and product improvements in a marketplace project built with Django + Next.js
...
- Align technical decisions with business constraints and operational needs

---

## How I Work

- Own backend services end-to-end: requirements → design → implementation → deployment → observability → production support
